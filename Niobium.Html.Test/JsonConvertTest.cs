...
{
    [Fact]
    public async void HtmlInterceptor_Test()
    {
        ...
        string htmlResult = await JsonObject.CreateHtml($"Test1", t => jObj.Convert(obj, t));
    ...
    [InlineData(1)]
    [InlineData(2)]
    public async void FileBasedTest(int ind)
    {
        string jsonText = File.ReadAllText($@"Data/{ind}.json");
        JObject obj = JsonConvert.DeserializeObject(jsonText) as JObject ?? throw new Exception("Json doesn't contain JObject");

        JsonObject jObj = new();
        var htmlResult = await JsonObject.CreateHtml($"Test-{ind}", t => jObj.Convert(obj, t));
